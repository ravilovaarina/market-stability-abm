...
2. **Experiment scripts** — `experiments/h1/` and `experiments/robustness/`
   - define populations, grids, metrics, plots, CSV output
3. **Outputs**
   - raw CSV files and plots under `results/h1/`
...
- Some interpretation tables and figures were added later than the original experiments; always distinguish raw simulation output from later analytical outputs.
...
| `h1-tipping-point` | H1 experiments v1-v6 (failed attempts with contrarian Chartist) | Speed attribute, fast/slow execution split, experiment files v1-v6 |
...
1D-ABM/
├── docs/
│   └── documentation.md             # Human-readable branch documentation
│
├── experiments/
│   ├── h1/
│   │   ├── main.py                  # Original experiment runner
│   │   ├── baseline_check.py        # Baseline diagnostic without HFT
│   │   ├── simulator_hft.py         # Front-running simulator used in v8
│   │   ├── experiment_h1.py         # H1 v1: baseline speed experiment
│   │   ├── experiment_h1_v2.py      # H1 v2: shock magnitude sweep + access asymmetry
│   │   ├── experiment_h1_v3.py      # H1 v3: direct metrics
│   │   ├── experiment_h1_v4.py      # H1 v4: increased to 30 runs
│   │   ├── experiment_h1_v5.py      # H1 v5: pure speed effect
│   │   ├── experiment_h1_v6.py      # H1 v6: fast_share x softlimit
│   │   ├── experiment_h1_v7.py      # H1 v7: delayed-price attempt
│   │   ├── experiment_h1_v8.py      # H1 v8: front-running attempt
│   │   ├── experiment_h1_v9.py      # H1 v9: TrendChartist result
│   │   └── experiment_unified.py    # Final unified H1 grids
│   │
│   └── robustness/
...
│           │   ├── shock_magnitude_agg.csv
│           │   ├── shock_magnitude_tipping.csv
...
│               ├── shock_magnitude_metrics.png
│               ├── shock_magnitude_heatmap.png
...
- **Total unified raw simulations:** `1,320 + 1,650 + 2,160 = 5,130` rows in `results/h1/raw/unified_all_raw.csv`

Note: when grouping `results/h1/raw/unified_all_raw.csv`, always include the `grid` column. Some parameter combinations, especially `speed_multiplier=1, info_lag=0`, appear in both Grid 1 and Grid 2 and therefore have 60 rows if grouped without `grid`.
...
| File | Contents |
|------|----------|
| `results/h1/raw/unified_speed_raw.csv` | Grid 1 raw results, 1,320 rows |
| `results/h1/raw/unified_delay_raw.csv` | Grid 2 raw results, 1,650 rows |
| `results/h1/raw/unified_combined_raw.csv` | Final Grid 3 raw results, 2,160 rows |
| `results/h1/raw/unified_all_raw.csv` | All unified grids concatenated, 5,130 rows |
| `results/h1/figures/unified_speed.png` | Grid 1 metrics + sensitivity table |
| `results/h1/figures/unified_delay.png` | Grid 2 metrics + heatmap |
| `results/h1/figures/unified_stats.png` | Mann-Whitney bar plots + sensitivity table |
...
The earlier files:
- `results/h1/robustness/raw/grid3_rerun_raw.csv`
- `results/h1/robustness/tables/grid3_rerun_agg.csv`
- `results/h1/robustness/raw/grid3_rerun_skips.csv`
- `results/h1/robustness/figures/grid3_rerun.png`
...
After `experiments/h1/experiment_unified.py` was completed, several follow-up scripts were added to strengthen interpretation and defense.
...
- by reusing `results/h1/raw/unified_all_raw.csv`.
...
Output files:
- `results/h1/robustness/tables/threshold_validation_summary.csv`
- `results/h1/robustness/figures/threshold_validation.png`
- `results/h1/robustness/figures/threshold_validation_heatmap.png`
...
- make Grid 3 directly comparable with Grid 1 and Grid 2 inside `results/h1/raw/unified_all_raw.csv`.

Final status:
- `results/h1/raw/unified_combined_raw.csv` contains `2,160` successful simulations,
...
Outputs:
- `results/h1/robustness/tables/speed_effect_mannwhitney.csv`
- `results/h1/robustness/tables/delay_effect_mannwhitney.csv`
- `results/h1/robustness/figures/speed_effect_mannwhitney.png`
- `results/h1/robustness/figures/delay_effect_mannwhitney.png`
...
Outputs:
- `results/h1/robustness/raw/noisy_delay_raw.csv` — raw simulation results,
- `results/h1/robustness/tables/noisy_delay_agg.csv` — aggregated results,
- `results/h1/robustness/tables/noisy_delay_tipping.csv` — tipping-point summary,
- `results/h1/robustness/figures/noisy_delay_metrics.png` — multi-metric line plots with bootstrap CI,
- `results/h1/robustness/figures/noisy_delay_heatmap.png` — heatmap of mean `vol_ratio`.

#### Results

The experiment completed successfully:
- total rows in `results/h1/robustness/raw/noisy_delay_raw.csv`: `1,650`,
...
Outputs:
- `results/h1/robustness/raw/shock_magnitude_raw.csv` — raw simulation results,
- `results/h1/robustness/tables/shock_magnitude_agg.csv` — aggregated results,
- `results/h1/robustness/tables/shock_magnitude_tipping.csv` — tipping-point summary by shock magnitude,
- `results/h1/robustness/figures/shock_magnitude_metrics.png` — multi-metric line plots with bootstrap CI,
- `results/h1/robustness/figures/shock_magnitude_heatmap.png` — heatmap of mean `vol_ratio`.

#### Results

The experiment completed successfully:
- total rows in `results/h1/robustness/raw/shock_magnitude_raw.csv`: `1,650`,
...
1. **Agent classes defined in experiment files, not in agents.py.** TrendChartist and SlowTrendChartist are defined inline in `experiments/h1/experiment_h1_v9.py` and `experiments/h1/experiment_unified.py`, not in the core `agents/agents.py`. This was a deliberate choice to avoid modifying the original codebase.
...
5. **`experiments/h1/experiment_h1_v7.py` has a bug** — the `delayed_price_lag` parameter is declared but never passed to the simulator. It's functionally identical to v6.

6. **`experiments/h1/simulator_hft.py` is separate from `AgentBasedModel/simulator/simulator.py`.** It has its own Simulator class with front-running logic. Only used in `experiments/h1/experiment_h1_v8.py`.

7. **All experiment files are standalone scripts.** Each `experiments/h1/experiment_h1_v*.py` file contains its own population setup, grid, run loop, metrics, and plotting. They import from `AgentBasedModel` but are self-contained.
...
# Final unified H1 experiment
python experiments/h1/experiment_unified.py

# Follow-up analyses that reuse existing raw results
python experiments/robustness/experiment_grid3_final_plots.py
python experiments/robustness/experiment_threshold_validation.py
python experiments/robustness/experiment_speed_delay_mannwhitney.py
python experiments/robustness/experiment_robustness_mannwhitney.py
```

The current scripts write outputs under `results/h1/`; historical H1 v1-v8 scripts write any newly generated files under `results/h1/legacy/`.
