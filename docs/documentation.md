# 1D-ABM Experiment Documentation
...
This document covers the current `h2-on-h1-volume-clock` branch. It records the H1 unified experiment, H2 event-time extensions, and the robustness analyses that support the course paper.

This branch is the working source for the H1/H2 part of the project. It contains:
- the original ABM framework and local market-microstructure fixes;
- the H1 unified experiment and its robustness checks;
- the H2 event-time / volume-clock experiments built on top of the H1 unified environment;
- raw result tables and figures needed to reproduce the H1/H2 conclusions.

This branch does **not** contain the final H3 implementation. H3 was developed on the separate
`h3-volatility-clustering` branch and should be treated as a separate line of evidence.

The old discussion notes and explanatory drafts are not part of the cleaned project structure. Their substantive
content is consolidated here in a neutral documentation format, so the repository can be understood from this file,
the experiment scripts, and the `results/` directory alone.
...
- `results/h1/raw/unified_speed_raw.csv`
- `results/h1/raw/unified_delay_raw.csv`
- `results/h1/raw/unified_combined_raw.csv`
- `results/h1/raw/unified_all_raw.csv`
...
- `experiments/h1/experiment_h1_v9.py`
- `results/h1/raw/h1_v9_raw.csv`

If you need the **post-unified follow-up analyses**, use:
- `experiments/robustness/experiment_threshold_validation.py`
- `experiments/robustness/experiment_grid3_rerun.py`
- `experiments/robustness/experiment_grid3_final_plots.py`
- `experiments/robustness/experiment_speed_delay_mannwhitney.py`
- `experiments/robustness/experiment_noisy_delay.py`
- `experiments/robustness/experiment_shock_magnitude.py`

If you need the **H2 analyses**, use:
- `experiments/h2/experiment_h2_event_time_unified.py`
- `experiments/h2/experiment_h2_postshock_volume_windows.py`
- `experiments/h2/experiment_h2_paired_stat_tests.py`
- `results/h2/raw/`
- `results/h2/figures/`

### Quick reading guide

For a fast reconstruction of the branch logic, read in this order:

1. **Model mechanics:** `Core Architecture`, then `Author's Modifications`.
2. **Why early H1 failed:** `The Sentiment Bug — Full Diagnosis`, then `H1 Experimental Progression`.
3. **Final H1 evidence:** `Implemented: Unified Experiment`, `Unified Experiment Results`, and:
   - `results/h1/raw/unified_speed_raw.csv`
   - `results/h1/raw/unified_delay_raw.csv`
   - `results/h1/raw/unified_combined_raw.csv`
   - `results/h1/figures/unified_speed.png`
   - `results/h1/figures/unified_combined_heatmap.png`
4. **H1 robustness:** `Post-Unified Follow-Up Work`, especially threshold validation and Mann-Whitney checks.
5. **Final H2 evidence:** `H2 on H1 Unified Environment`, then:
   - `results/h2/raw/h2_unified_calibrated_tipping.csv`
   - `results/h2/figures/h2_unified_calibrated_tipping.png`
6. **Strict H2 clock comparison:** `H2 Improvement on H1: Equal Post-Shock Volume Windows`, then:
   - `results/h2/raw/h2_postshock_volume_paired_diffs.csv`
   - `results/h2/raw/h2_paired_stat_tests.csv`
   - `results/h2/figures/h2_paired_stat_tests.png`

The key interpretive rule is conservative: do not claim that "more speed always destabilizes the market" or that
"event time always stabilizes the market." Both effects are regime-dependent.
...
- Early files `experiments/h1/experiment_h1_v1-v8.py` are historical and not the final modeling logic.
...
- The final Grid 3 result is in `results/h1/raw/unified_combined_raw.csv` and is included in `results/h1/raw/unified_all_raw.csv`.
...
|--------|---------|-------------|
| `main` | Original baseline simulator from bognik002/1D-ABM with local bug fixes | Base ABM framework. |
| `h1-tipping-point` | Early H1 experiment history | H1 exploratory versions and Russian H1 description. |
| `hft-intraiter` | H1 development branch | TrendChartist, SlowTrendChartist, unified H1 experiment, robustness checks. |
| `h2-event-time-volume-clock` | Standalone H2 prototype | Event-time / volume-clock prototype before porting into the H1 unified environment. |
| `h2-on-h1-volume-clock` | Current branch | H1 unified environment plus final H2-on-H1 event-time experiments. |

**Active development branch:** `h2-on-h1-volume-clock`.
...
├── experiments/
│   ├── paths.py                     # Shared project-root and result-path helpers
│   ├── h1/
│   │   ├── baseline_check.py
│   │   ├── experiment_h1.py         # H1 v1
│   │   ├── experiment_h1_v2.py      # H1 v2
│   │   ├── experiment_h1_v3.py      # H1 v3
│   │   ├── experiment_h1_v4.py      # H1 v4
│   │   ├── experiment_h1_v5.py      # H1 v5
│   │   ├── experiment_h1_v6.py      # H1 v6
│   │   ├── experiment_h1_v7.py      # H1 v7, historical delay prototype
│   │   ├── experiment_h1_v8.py      # H1 v8, front-running prototype
│   │   ├── experiment_h1_v9.py      # H1 v9, pre-unified TrendChartist experiment
│   │   ├── experiment_unified.py    # Final H1 unified experiment
│   │   └── simulator_hft.py         # Front-running simulator used by v7/v8
│   ├── h2/
│   │   ├── experiment_h2_event_time_unified.py
│   │   ├── experiment_h2_postshock_volume_windows.py
│   │   └── experiment_h2_paired_stat_tests.py
│   └── robustness/
...
├── results/
│   ├── h1/
│   │   ├── raw/                     # H1 and unified raw CSV tables
│   │   └── figures/                 # H1 and unified plots
│   ├── h2/
│   │   ├── raw/                     # H2 raw, aggregate, paired-test CSV tables
│   │   └── figures/                 # H2 plots
│   └── robustness/
│       ├── raw/                     # robustness CSV tables
...
**Order Book Population:** V=1000 orders are sampled with prices drawn from N(p0-sigma, sigma) for bids and N(p0+sigma, sigma) for asks, quantities uniform [1,5].
...
```
where sigma_d = 5e-3. Initial dividend = rf * p0. The exchange maintains a rolling list of 100 pre-sampled future dividends. Each iteration consumes one and samples a new one.
...
**File:** `experiments/h1/experiment_h1_v9.py` (defined inline in the experiment file)
...
**File:** `experiments/h1/experiment_h1_v9.py`
...
### Modification 5: Front-Running Simulator (experiments/h1/simulator_hft.py)

**File:** `experiments/h1/simulator_hft.py` (NEW file, used only in experiments/h1/experiment_h1_v8.py)
...
- **Change:** Uses `experiments/h1/simulator_hft.py` with `front_running=True`. Slow orders intercepted.
...
## Implemented: Unified Experiment (`experiments/h1/experiment_unified.py`)
...
The final Grid 3 rerun is stored in `results/h1/raw/unified_combined_raw.csv` and included in `results/h1/raw/unified_all_raw.csv`. This rerun replaced the earlier partial `grid3_rerun_*` attempt as the source of truth for the combined regime.
...
After `experiments/h1/experiment_unified.py` was completed, several follow-up scripts and presentation materials were added to strengthen interpretation and defense.

### Follow-up 1: Threshold validation (`experiments/robustness/experiment_threshold_validation.py`)
...
Output files:
- `results/robustness/raw/threshold_validation_summary.csv`
- `results/robustness/figures/threshold_validation.png`
- `results/robustness/figures/threshold_validation_heatmap.png`
...
#### Historical note: earlier targeted Grid 3 rerun (`experiments/robustness/experiment_grid3_rerun.py`)
...
### Follow-up 3: Additional Mann-Whitney analyses (`experiments/robustness/experiment_speed_delay_mannwhitney.py`)
...
Outputs:
- `results/robustness/raw/speed_effect_mannwhitney.csv`
- `results/robustness/raw/delay_effect_mannwhitney.csv`
- `results/robustness/figures/speed_effect_mannwhitney.png`
- `results/robustness/figures/delay_effect_mannwhitney.png`

### Follow-up 4: Noisy-agent information delay experiment (`experiments/robustness/experiment_noisy_delay.py`)
...
### Follow-up 5: Shock magnitude robustness experiment (`experiments/robustness/experiment_shock_magnitude.py`)
...
```text
results/h2/raw/h2_unified_calibrated_raw.csv
results/h2/raw/h2_unified_calibrated_agg.csv
results/h2/raw/h2_unified_calibrated_tipping.csv
results/h2/raw/h2_unified_calibrated_stats.csv
results/h2/figures/h2_unified_calibrated_metrics.png
results/h2/figures/h2_unified_calibrated_heatmap.png
results/h2/figures/h2_unified_calibrated_tipping.png
```
...
```bash
python3 experiments/h2/experiment_h2_event_time_unified.py --runs 2 --n-iter 120 --shock-it 50 --hft-frac 0 0.5 --no-plots
```

Final candidate:

```bash
python3 experiments/h2/experiment_h2_event_time_unified.py
```

Calendar-only sanity check:

```bash
python3 experiments/h2/experiment_h2_event_time_unified.py --calendar-only --no-plots
```

High-activity sensitivity:

```bash
python3 experiments/h2/experiment_h2_event_time_unified.py --no-calibrate-vstar --vstar 25 50 100 --output-prefix h2_unified_high_activity
```
...
```bash
python3 experiments/h2/experiment_h2_event_time_unified.py --plot-from-raw
```

This second command does not rerun simulations. It recomputes aggregation, tipping, Mann-Whitney statistics,
and PNG figures from `results/h2/raw/h2_unified_calibrated_raw.csv`.
...
```text
results/h2/raw/h2_postshock_volume_raw.csv
results/h2/raw/h2_postshock_volume_agg.csv
results/h2/raw/h2_postshock_volume_paired_diffs.csv
results/h2/raw/h2_postshock_volume_diff_summary.csv
results/h2/figures/h2_postshock_volume_metrics.png
results/h2/figures/h2_postshock_volume_diffs.png
```
...
`experiments/h2/experiment_h2_paired_stat_tests.py`. It does not run new simulations. It reads
...
#### How to read the H2 plots

`results/h2/figures/h2_unified_calibrated_tipping.png` shows the estimated tipping point `phi*`
for calendar time and for each event-time volume threshold. A lower bar means the market crosses the
1.3× instability threshold at a smaller HFT share. The important comparison is:

```text
calendar phi* = 0.2
event_time_V13 phi* = 0.2
```

`V13` is the closest calibrated event-time clock because the baseline calendar volume per tick is about
12.915. Therefore, `V13` represents approximately the same executed volume as one calendar tick.
The fact that `phi*` remains 0.2 under `V13` is the main evidence that the H1 tipping result is not just
a calendar-time artifact.

`results/h2/figures/h2_paired_stat_tests.png` shows paired event-time-minus-calendar differences.
For instability metrics:

```text
negative bar = event time lower / more stable
positive bar = event time higher / less stable
confidence interval crossing zero = mixed or statistically weak evidence
```

The figure should be read metric by metric rather than as a single universal verdict. Event time lowers
some post/pre volatility-ratio measures, especially at `phi=0.0`, `phi=0.8`, and `phi=1.0`, but it does
not uniformly lower realized variance per volume, realized variance per trade, or volume-normalized
volatility across all HFT shares.

### Technical fix: plotting bug in `experiments/h1/experiment_unified.py`
...
1. **Agent classes defined in experiment files, not in agents.py.** TrendChartist and SlowTrendChartist are defined inline in `experiments/h1/experiment_h1_v9.py`, not in the core `agents/agents.py`. This was a deliberate choice to avoid modifying the original codebase.
...
6. **experiments/h1/experiment_h1_v7.py has a bug** — the `delayed_price_lag` parameter is declared but never passed to the simulator. It's functionally identical to v6.

7. **experiments/h1/simulator_hft.py is separate from simulator.py.** It has its own Simulator class with front-running logic. Only used in experiments/h1/experiment_h1_v8.py.
...
# H1 experiment v9
python experiments/h1/experiment_h1_v9.py

# Final H1 unified experiment
python experiments/h1/experiment_unified.py

# H2 event-time experiment
python experiments/h2/experiment_h2_event_time_unified.py
```
