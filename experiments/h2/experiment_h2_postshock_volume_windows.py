#!/usr/bin/env python3
"""
H2-on-H1 improved: post-shock volume-window comparison.
...
The implementation intentionally reuses the H1 unified population and
fast-first trading logic through experiment_unified.py, so it belongs to the
h2-on-h1-volume-clock branch rather than to the standalone H2 prototype.
...
from AgentBasedModel.agents import Chartist
from AgentBasedModel.events import MarketPriceShock
from AgentBasedModel.simulator import SimulatorInfo

from experiments.h2.experiment_h2_event_time_unified import VolumeExchangeAgent
from experiments.h1.experiment_unified import create_population, spread_ratio, vol_ratio
...
class LocalSimulator:
    """Small simulator shell for logged calendar/event-time H2-on-H1 runs."""
...
def update_behaviour(sim: LocalSimulator) -> None:
...
def call_traders_fast_first(traders, speed_multiplier: int = 1) -> None:
...
def apply_payments(sim: LocalSimulator) -> None:
...
def price_vol_ratio(info, shock_tick: int, window: int) -> float:
    return vol_ratio(info, shock_it=shock_tick, window=window)


def mm_stress_ratio(info, shock_tick: int, softlimit: int) -> float:
    del softlimit
    return info.mm_panic_ratio(from_it=shock_tick)
...
def compute_volume_window_metrics(
    ...
    *,
    ...
    return {
        ...
        "spread_ratio": spread_ratio(logged.sim.info, shock_tick),
...
def output_paths(prefix: str) -> Dict[str, Path]:
    return {
        "raw": result_path("h2", "raw", f"{prefix}_raw.csv"),
        "agg": result_path("h2", "raw", f"{prefix}_agg.csv"),
        "diffs": result_path("h2", "raw", f"{prefix}_paired_diffs.csv"),
        "diff_summary": result_path("h2", "raw", f"{prefix}_diff_summary.csv"),
        "metrics_png": result_path("h2", "figures", f"{prefix}_metrics.png"),
        "diffs_png": result_path("h2", "figures", f"{prefix}_diffs.png"),
    }
