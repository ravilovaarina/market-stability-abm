...
if __name__ == '__main__':
    ...
    speed_raw = result_path("h1", "raw", "unified_speed_raw.csv")
    delay_raw = result_path("h1", "raw", "unified_delay_raw.csv")
    combined_raw = result_path("h1", "raw", "unified_combined_raw.csv")
    all_raw = result_path("h1", "raw", "unified_all_raw.csv")

    # Grid 1: Load existing results
    if speed_raw.exists():
        print(f'\n[1/4] Grid 1: Loading from {speed_raw}...')
        df_speed = pd.read_csv(speed_raw)
        ...
        df_speed.to_csv(speed_raw, index=False)
        print(f'Saved: {speed_raw}')

    # Grid 2: Load existing results
    if delay_raw.exists():
        print(f'\n[2/4] Grid 2: Loading from {delay_raw}...')
        df_delay = pd.read_csv(delay_raw)
        ...
        df_delay.to_csv(delay_raw, index=False)
        print(f'Saved: {delay_raw}')

    # Grid 3: Run (or load if exists)
    if combined_raw.exists():
        print(f'\n[3/4] Grid 3: Loading from {combined_raw}...')
        df_combined = pd.read_csv(combined_raw)
        print(f'  Loaded {len(df_combined)} rows')
    else:
        print('\n[3/4] Grid 3: Combined...')
        df_combined = run_grid_combined(N_RUNS, N_ITER, SHOCK_IT, SHOCK_DP, SOFTLIMIT)
        df_combined.to_csv(combined_raw, index=False)
        print(f'Saved: {combined_raw}')

    # Merge and analyze
    df_all = pd.concat([df_speed, df_delay, df_combined], ignore_index=True)
    df_all.to_csv(all_raw, index=False)
    print(f'Saved: {all_raw}')
    ...
    plot_grid_speed(df_all, agg, save=result_path("h1", "figures", "unified_speed.png"))
    plot_grid_delay(df_all, agg, save=result_path("h1", "figures", "unified_delay.png"))
    plot_stat_tests(df_all, save=result_path("h1", "figures", "unified_stats.png"))
