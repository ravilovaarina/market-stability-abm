...
Uses existing `results/h3/confirmatory/raw/h3_confirmatory_raw.csv`.
No new simulations are needed.
...
Statistical test:
  OLS with HC3 standard errors, matching the confirmatory OLS design.
...
Outputs:
  ...
  - results/h3/shock_absorption/tables/h3_shock_absorption_means.csv
  - results/h3/shock_absorption/figures/h3_shock_absorption.png
"""
...
CONFIRMATORY_ROOT = PROJECT_ROOT / "results" / "h3" / "confirmatory"
OUTPUT_ROOT = PROJECT_ROOT / "results" / "h3" / "shock_absorption"
RAW_CSV = str(CONFIRMATORY_ROOT / "raw" / "h3_confirmatory_raw.csv")
OLS_CSV = str(OUTPUT_ROOT / "tables" / "h3_shock_absorption_ols.csv")
MW_CSV = str(OUTPUT_ROOT / "tables" / "h3_shock_absorption_mw.csv")
MEANS_CSV = str(OUTPUT_ROOT / "tables" / "h3_shock_absorption_means.csv")
PLOT_PNG = str(OUTPUT_ROOT / "figures" / "h3_shock_absorption.png")
...
def main() -> None:
    ...
    print("\nFitting OLS HC3...")
    ols = fit_ols_hc3(raw)
    ols.to_csv(OLS_CSV, index=False)
    print(f"  Saved: {OLS_CSV}")
