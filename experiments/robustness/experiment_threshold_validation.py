...
def load_raw(base_dir: str | Path) -> pd.DataFrame:
    path = Path(base_dir) / "unified_all_raw.csv"
    if not path.exists():
...
def main() -> None:
    df = load_raw(H1_RAW_ROOT)

    summary = build_summary(df)

    csv_out = ROBUSTNESS_ROOT / "tables" / "threshold_validation_summary.csv"
    plot_out = ROBUSTNESS_ROOT / "figures" / "threshold_validation.png"
    heatmap_out = ROBUSTNESS_ROOT / "figures" / "threshold_validation_heatmap.png"
