...
def load_raw() -> pd.DataFrame:
    path = result_path("h1", "raw", "unified_all_raw.csv")
    if not path.exists():
        raise FileNotFoundError(
            f"Cannot find {path}. Run experiment_unified.py first."
        )
...
def main() -> None:
    ...
    csv_out = result_path("robustness", "raw", "threshold_validation_summary.csv")
    plot_out = result_path("robustness", "figures", "threshold_validation.png")
    heatmap_out = result_path("robustness", "figures", "threshold_validation_heatmap.png")
