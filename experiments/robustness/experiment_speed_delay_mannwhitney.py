...
H1_RAW_ROOT = PROJECT_ROOT / "results" / "h1" / "raw"
ROBUSTNESS_ROOT = PROJECT_ROOT / "results" / "h1" / "robustness"

def load_data(base_dir: str | Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    speed_path = Path(base_dir) / "unified_speed_raw.csv"
    delay_path = Path(base_dir) / "unified_delay_raw.csv"
...
def write_markdown_report(speed_results: pd.DataFrame, delay_results: pd.DataFrame, out_path: str) -> None:
    lines = [
    ...
    for _, row in speed_results.iterrows():
        p = row["p_value"]
        lines.append(
            f"| {row['phi']:.1f} | {int(row['baseline_speed'])} | {int(row['speed_mult'])} | "
            f"{row['baseline_mean']:.3f} | {row['treatment_mean']:.3f} | "
            f"{p:.4g} | {'yes' if row['significant'] else 'no'} |"
        )
    ...
    for _, row in delay_results.iterrows():
        p = row["p_value"]
        lines.append(
            f"| {row['phi']:.1f} | {int(row['baseline_lag'])} | {int(row['info_lag'])} | "
            f"{row['baseline_mean']:.3f} | {row['treatment_mean']:.3f} | "
            f"{p:.4g} | {'yes' if row['significant'] else 'no'} |"
        )
...
def main() -> None:
    df_speed, df_delay = load_data(H1_RAW_ROOT)

    speed_results = analyze_speed_effect(df_speed)
    delay_results = analyze_delay_effect(df_delay)

    speed_csv = ROBUSTNESS_ROOT / "tables" / "speed_effect_mannwhitney.csv"
    delay_csv = ROBUSTNESS_ROOT / "tables" / "delay_effect_mannwhitney.csv"
    speed_png = ROBUSTNESS_ROOT / "figures" / "speed_effect_mannwhitney.png"
    delay_png = ROBUSTNESS_ROOT / "figures" / "delay_effect_mannwhitney.png"
