...
H1_FIGURE_ROOT = PROJECT_ROOT / "results" / "h1" / "figures"
ROBUSTNESS_TABLE_ROOT = PROJECT_ROOT / "results" / "h1" / "robustness" / "tables"


def load_grid3(base_dir: str | Path) -> pd.DataFrame:
    path = Path(base_dir) / "unified_combined_raw.csv"
    if not path.exists():
        raise FileNotFoundError(
            f"Missing {path}. Run experiment_unified.py or place unified_combined_raw.csv in the repo."
        )
...
def main() -> None:
    df = load_grid3(H1_RAW_ROOT)
    tipping = build_tipping_table(df)

    tipping_path = ROBUSTNESS_TABLE_ROOT / "unified_combined_tipping.csv"
    metrics_path = H1_FIGURE_ROOT / "unified_combined_metrics.png"
    heatmap_path = H1_FIGURE_ROOT / "unified_combined_heatmap.png"
