...
Robustness experiment for H1:
vary the exogenous market shock magnitude while keeping info_lag=0.

Purpose:
- test whether the HFT tipping point is stable across shock sizes,
- distinguish a market-structure threshold from a shock-size artifact.

Grid:
- shock_dp in {-0.5, -1, -2, -5, -10}
- hft_frac in {0.0, 0.1, ..., 1.0}
- 30 runs per parameter combination by default

Default speed setting:
- speed_multiplier=2, matching the cleanest unified speed-grid result.
- use --speed-multiplier 1 for a v9-style priority-only comparison.
...
ROBUSTNESS_ROOT = PROJECT_ROOT / "results" / "h1" / "robustness"
...
class TrendChartist(Chartist):
    ...
    This experiment fixes information delay at zero, so both fast and slow
    chartists observe real-time price changes. Only execution speed and shock
    magnitude vary.
    """
...
class SimulatorShockMagnitude:
    """
    Simulator with H1 fast/slow activation and no information delay.

    Fast agents act first and may act `speed_multiplier` times per iteration.
    """
    ...
    def simulate(self, n_iter, silent=False, speed_multiplier=2):
...
def create_population(
    ...
    for _ in range(n_random):
        trader = Random(exchange, cash=10**3, info_lag=0)
        trader.speed = "slow"
        traders.append(trader)
...
def run_grid(
    ...
    total = len(SHOCK_DPS) * len(HFT_FRACS) * n_runs
    print(
        f"Shock-magnitude grid: {len(SHOCK_DPS)} shocks × {len(HFT_FRACS)} phi values × {n_runs} runs = {total} runs"
    )
    print(f"Fixed info_lag=0, speed_multiplier={speed_multiplier}")
    if completed:
        print(f"Resume enabled: {len(completed)} runs already recorded and will be skipped.")

    for shock_dp in SHOCK_DPS:
        for phi in tqdm(HFT_FRACS, desc=f"shock_dp={shock_dp}"):
            for run in range(n_runs):
                key = (float(shock_dp), float(phi), int(speed_multiplier), int(run))
                if key in completed:
                    continue

                shock_code = int(abs(shock_dp) * 100)
                seed = run * 10000 + int(phi * 10) * 100 + shock_code + speed_multiplier * 1000
                row = run_one(
                    phi,
                    shock_dp=shock_dp,
                    n_iter=n_iter,
...
def aggregate(df):
    return (
        df.groupby(["grid", "speed_mult", "shock_dp", "hft_frac"])
        .agg(
...
def build_tipping_table(df):
    ...
    return pd.DataFrame(rows).sort_values("shock_dp", ascending=False)
...
def plot_metrics(df_raw, save_path):
    ...
    colors = plt.cm.viridis(np.linspace(0.15, 0.85, len(SHOCK_DPS)))
    ...
    for idx, (metric, title) in enumerate(metrics):
        ...
        for shock_dp, color in zip(SHOCK_DPS, colors):
            sub = df_raw[df_raw["shock_dp"] == shock_dp]
    ...
    table = ax.table(
        cellText=table_rows,
        colLabels=["Shock", "Baseline", "1.3x", "phi*", "Max"],
        cellLoc="center",
        loc="center",
    )
    ...
    speed_mult = int(df_raw["speed_mult"].iloc[0])
    fig.suptitle(
        "Shock magnitude robustness experiment\n"
        f"info_lag=0 | speed_multiplier={speed_mult} | 30 runs | Shading = 95% bootstrap CI",
        fontsize=12,
        fontweight="bold",
    )
...
def plot_heatmap(df_raw, tipping, save_path):
    pivot = (
        df_raw.groupby(["shock_dp", "hft_frac"])["vol_ratio"]
        .mean()
        .unstack("hft_frac")
        .reindex(SHOCK_DPS)
    )
...
def main():
    parser = argparse.ArgumentParser(description="Run shock magnitude robustness experiment.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Fast chartist calls per iteration. Default 2 matches the clean unified speed result.",
    )
    parser.add_argument("--no-resume", action="store_true")
    args = parser.parse_args()

    raw_out = ROBUSTNESS_ROOT / "raw" / "shock_magnitude_raw.csv"
    agg_out = ROBUSTNESS_ROOT / "tables" / "shock_magnitude_agg.csv"
    tipping_out = ROBUSTNESS_ROOT / "tables" / "shock_magnitude_tipping.csv"
    metrics_png = ROBUSTNESS_ROOT / "figures" / "shock_magnitude_metrics.png"
    heatmap_png = ROBUSTNESS_ROOT / "figures" / "shock_magnitude_heatmap.png"
