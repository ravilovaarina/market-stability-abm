...
def main() -> None:
    # ── 1. Shock magnitude ────────────────────────────────────────────────────
    print("Loading shock_magnitude_raw.csv...")
    sm = pd.read_csv("shock_magnitude_raw.csv")
    print(f"  {len(sm)} rows")

    print("Running Mann-Whitney for shock_magnitude...")
    sm_result = mann_whitney_vs_baseline(sm, group_col="shock_dp")
    sm_result.to_csv("robustness_mw_shock_magnitude.csv", index=False)
    print("  Saved: robustness_mw_shock_magnitude.csv")
    ...
    plot_results(
        ...
        output_path="robustness_mw_shock_magnitude.png",
    )

    # ── 2. Noisy delay ────────────────────────────────────────────────────────
    print("\nLoading noisy_delay_raw.csv...")
    nd = pd.read_csv("noisy_delay_raw.csv")
    print(f"  {len(nd)} rows")

    print("Running Mann-Whitney for noisy_delay...")
    nd_result = mann_whitney_vs_baseline(nd, group_col="info_lag")
    nd_result.to_csv("robustness_mw_noisy_delay.csv", index=False)
    print("  Saved: robustness_mw_noisy_delay.csv")
    ...
    plot_results(
        ...
        output_path="robustness_mw_noisy_delay.png",
    )
